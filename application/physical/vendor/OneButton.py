...
class OneButton:
  ...
  def tick(self):
    ...
    if self._state == 0: # waiting for menu pin being pressed.
      ...
      if (buttonLevel == self._buttonReleased) and (ticks_diff(now,self._startTime) < self._debounceTicks):
        # button was released to quickly so I assume some debouncing.
        # go back to state 0 without calling a function.
        #print(ticks_diff(now,self._startTime),self._debounceTicks )
        self._state = 0
      ...
      if ticks_diff(now,self._startTime) > self._clickTicks:
